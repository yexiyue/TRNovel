...
字段详解见[书源结构](/TRNovel/book-source/structure),规则语法见[规则语法](/TRNovel/book-source/rules)。规则的取值后端支持 **CSS / XPath / JSONPath / 正则**;`clean` 流水线内置 **加解密(AES/DES)、编解码(base64/hex/url)、哈希(md5/sha/hmac)、繁简转换**,足以应对正文加密与 URL 签名等反爬;极少数动态场景另有 **JS 逃生舱**。
