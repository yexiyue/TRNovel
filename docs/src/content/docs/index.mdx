...
<CardGrid>
	...
	<Card title="强大的书源规则引擎 🆕" icon="puzzle">
		取值后端 CSS / XPath / JSONPath / 正则任选;`clean` 流水线内置 AES/DES 加解密、Base64/Hex/URL 编解码、MD5/SHA/HMAC、繁简转换 —— 正文加密、URL 签名这类反爬,纯声明式即可拿下;实在动态的还有 JS 逃生舱。
	</Card>
	<Card title="反爬 / 浏览器辅助 🆕" icon="setting">
		撞上 Cloudflare?复用系统已装浏览器解挑战、领 `cf_clearance` 再交回快请求 —— 能自动就自动,必要时点一下「我是真人」即可。正文若被加密,结合规则引擎的加解密算子一并还原。
	</Card>
