# 从终端阅读器到 AI 书源工厂：TRNovel 是怎么与时俱进的
...
> 在 AI Agent 已经能读网页、写代码、跑命令的时代，一个开源小说阅读器应该怎么进化？
...
- [第一层进化：安装方式要跟上普通用户](#第一层进化安装方式要跟上普通用户)
- [第二层进化：本地 TXT 阅读不应该只是正则切章节](#第二层进化本地-txt-阅读不应该只是正则切章节)
- [第三层进化：网络书源从 Legado 风格走向 v2 结构化格式](#第三层进化网络书源从-legado-风格走向-v2-结构化格式)
- [第四层进化：书源不能只靠猜，要能体检](#第四层进化书源不能只靠猜要能体检)
- [第五层进化：通过 skill 创建书源](#第五层进化通过-skill-创建书源)
- [第六层进化：反爬不能装作不存在](#第六层进化反爬不能装作不存在)
- [现在如何安装和体验](#现在如何安装和体验)
- [后面还想做什么](#后面还想做什么)
...
## 第二层进化：本地 TXT 阅读不应该只是正则切章节

小说 TXT 文件看似简单，实际上非常脏。
...
以前很多阅读器处理本地 TXT 的方式是：写一条大正则，匹配到了就算章节。
...
- 有些书有分卷，有些书没有；
- 有些标题像章节，其实只是正文里的一句话；
- 有些规则需要前后向断言，Rust 标准 `regex` 又不支持。

TRNovel 最近重写了本地目录识别逻辑：章节识别不再只是「扫到正则就切」，而是有一套规则集和启发式判断。
...
- 目录按「卷 -> 章」展示；
- 支持折叠和搜索；
- 支持用户自定义 `~/.novel/toc_rules.json`；
- 规则字段对齐 Legado 的 `TxtTocRule`，可以复用社区里一部分成熟的本地 TXT 章节规则。

这一层升级解决的是本地阅读体验。

本地 TXT 阅读本来应该是一个安静的功能：你不应该感知到它很复杂，你只应该打开文件，然后看到一个正常的目录。

## 第三层进化：网络书源从 Legado 风格走向 v2 结构化格式
